...
/*! Required | Ezoic *//*! TOS Event | Ezoic */
function ez_TOS_TrackEvent(e) {
    if (typeof ez_tos_track_count !== 'undefined') {
        ez_last_activity_count = ez_tos_track_count;
    }
}
ez_attachEvent(window, 'scroll', ez_TOS_TrackEvent);
ez_attachEvent(document, 'mousemove', ez_TOS_TrackEvent);
ez_attachEvent(document, 'keyup', ez_TOS_TrackEvent);
/* end: TOS *//*! EzQueue | Ezoic */
...
var ezux = (function () {
    ...
    var pixels = {
        ...
        addLocalTime: function () {
            ...
            var localDate = (new Date(now - (tzOffset * 60000))).toISOString().slice(0, 19).replace('T', ' ').split(' ')[0];
            var localHour = now.getHours();
            var localDay = now.getDay();
            var localTZ = -tzOffset / 60;
            ...
            px.AddPVPixel(pvID, [(new EzoicPixelData('t_local_timezone', localTZ))]);
        },
    ...
    function attachListeners() {
        ...
        if (document.addEventListener) {
            ...
            document.addEventListener(visibilityChangeEventName, function (e) {
                if (document[hiddenPropName]) evts.unload(e);
            }, false);
        } else {
